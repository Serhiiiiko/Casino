<template>
                ...
                <div class="flex items-center justify-start py-3">
                    ...
                    <a v-if="setting" href="/" class="flex ml-2 md:mr-24">
                        ...
                        <div class="block sm:hidden">
                            <img
                                :src="`/storage/` + setting.software_favicon"
                                class="h-10 mr-3"
                                alt=""
                            />
                        </div>
                ...
                <div v-if="!simple" class="flex items-center py-3">
                    <div v-if="!isAuthenticated" class="flex ml-5">
                        ...
                        <button
                            ...
                            class="ui-button-blue ml-3 mr-3 rounded"
                        >
                            {{ $t('Register') }}
                        </button>
                    </div>
                    ...
                    <div class="mb-5 w-full">
                        <div class="md:w-4/6 2xl:w-4/6 mx-auto">
                            ...
                            >
                                ...
                                <p
                                    class="font-normal text-gray-700 dark:text-gray-400 opacity-20"
                                >
                ...
                <div class="w-full relative p-5">
                    ...
                    <form @submit.prevent="loginSubmit" method="post" action="" class="">
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-300 mb-6">
                            Новая запись?
                            <a href="" @click.prevent="hideLoginShowRegisterToggle"
                                ><strong>Зарегистрироваться</strong></a
                            >
                        </p>
                    </form>
                </div>
            ...
            <div class="flex md:justify-between h-full">
                ...
                <div class="w-full relative p-5 m-auto">
                    <form @submit.prevent="registerSubmit" method="post" action="" class="">
                        ...
                        <!-- CPF убираем из формы (закомментировано):
                        <div class="relative mb-3">
    ...
    <!-- Модальное окно профиля (без изменений, для примера) -->
    <div
        id="modalProfileEl"
        tabindex="-1"
        aria-hidden="true"
...
export default {
  ...
  data() {
    return {
      ...
      registerForm: {
        name: "",
        email: "",
        // сохраняем поле cpf, но не показываем его в форме
        cpf: "",
        password: "",
  ...
  async created() {
    // При создании компонента генерируем случайный CPF
  ...
  methods: {
     ...
     * Если нужно просто 11-значное число, можно упростить.
     */
    generateRandomCPF() {
      ...
      // Вычислим первые два проверочных разряда
      let d1 = 0;
      let d2 = 0;
      for (let i = 0; i < 9; i++) {
      ...
      await HttpApi.post("auth/register", _this.registerForm)
        .then((response) => {
          if (response.data.access_token !== undefined) {
            ...
            // Сбрасываем форму (CPF сгенерируется заново при заходе на форму/обновлении)
            _this.registerForm = {
              name: "",
              email: "",
              cpf: "",
