...
<template>
    <aside
      ...
      aria-label="Sidebar"
    >
      <div class="h-full pb-4 overflow-y-auto sidebar-color p-4 flex flex-col">
        <div class="flex items-center justify-end mb-4">
          <button
            @click="toggleCollapse"
            class="p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-800 transition-all duration-300"
          >
            <svg v-if="!isCollapsed" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
              <path
                d="M12.146 4.646a.5.5 0 0 1 .708.708L9.707 8l3.147 3.146a.5.5 0 0 1-.708.708l-3.5-3.5a.5.5 0 0 1 0-.708l3.5-3.5z"
              />
            </svg>
            <svg v-else width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
              <path
                d="M8.354 4.646a.5.5 0 0 1 0 .708L5.207 8l3.147 3.146a.5.5 0 0 1-.708.708l-3.5-3.5a.5.5 0 0 1 0-.708l3.5-3.5a.5.5 0 0 1 .708 0z"
              />
            </svg>
          </button>
        </div>
        <ul>
          <li class="mb-3">
            <div class="flex justify-between w-full gap-4">
              ...
              <button
                v-if="!isAuthenticated && setting && setting.disable_spin"
                @click.prevent="$router.push('/landing/spin')"
                class="btn-menu-rotate border border-red-500"
              >
                <div v-if="!isCollapsed">
                  <h1>{{ $t('Rotate') }}</h1>
                </div>
                <div class="spin-anim">
                  <img class="img-spinbg" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAA..." alt="">
                  <img class="img-turntable" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAA..." alt="">
                  <img class="img-pointer" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAC..." alt="">
                </div>
              </button>
              <button v-if="isAuthenticated" @click.prevent="$router.push('/profile/affiliate')" class="btn-refer border border-yellow-300">
                <div v-if="!isCollapsed">
            ...
            >
              <div class="flex justify-center text-center self-center items-center">
                <img
                  ...
                  class="mr-3"
                />
                <span v-if="!isCollapsed">{{ $t('Double your balance') }}</span>
                <span v-else>...</span>
              </div>
            </button>
          </li>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            >
              <img :src="`/assets/images/icons/home.svg`" alt="" width="35">
              <span v-if="!isCollapsed" class="text-[12px] mt-2">{{ $t('Home') }}</span>
            </RouterLink>
          </div>
          <div>
            <RouterLink
            ...
            >
              <img :src="`/assets/images/icons/fortune.svg`" alt="" width="35">
              <span v-if="!isCollapsed" class="text-[12px] mt-2">{{ $t('Casino') }}</span>
            </RouterLink>
          </div>
        </div>
        <div class="mt-4">
          <div class="grid grid-cols-2 gap-4 mb-4" v-if="isLoading">
            <div v-for="i in 10" :key="i" class="flex items-center justify-center h-20 max-w-sm rounded-xl animate-pulse bg-gray-100 dark:bg-gray-700"></div>
          </div>
          <div v-else class="grid grid-cols-2 gap-4 mt-4 mb-4">
            <RouterLink
            ...
            >
              <img :src="`/storage/` + category.image" alt="" width="35">
              <span v-if="!isCollapsed" class="text-[12px] mt-2">{{ $t(category.name) }}</span>
            </RouterLink>
          </div>
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            >
              <img :src="`/assets/images/icons/wallet-money.svg`" alt="" width="35">
              <span v-if="!isCollapsed" class="text-[12px] mt-2">{{ $t('Wallet') }}</span>
            </RouterLink>
          </div>
          <div>
            <RouterLink
            ...
            >
              <img :src="`/assets/images/icons/folder-favourite.svg`" alt="" width="35">
              <span v-if="!isCollapsed" class="text-[12px] mt-2">{{ $t('Favorites') }}</span>
            </RouterLink>
          </div>
        </div>
        <div class="grid grid-cols-2 gap-4 mt-4">
          <div>
            <RouterLink :to="{ name: 'eventsPage' }" active-class="category-action" class="relative">
              <img :src="`/assets/images/eventos.png`" alt="" class="w-full">
              <h1 v-if="!isCollapsed" class="button-title-menu">События</h1>
            </RouterLink>
          </div>
          <div>
            <RouterLink :to="{ name: 'recentsPage' }" active-class="category-action" class="relative">
              <img :src="`/assets/images/juros.png`" alt="" class="w-full">
              <h1 v-if="!isCollapsed" class="button-title-menu">{{ $t('Recents') }}</h1>
            </RouterLink>
          </div>
          <div>
            <RouterLink :to="{ name: 'awardsPage' }" active-class="category-action" class="relative">
              <img :src="`/assets/images/pendente.png`" alt="" class="w-full">
              <h1 v-if="!isCollapsed" class="button-title-menu">{{ $t('Awards') }}</h1>
            </RouterLink>
          </div>
          <div>
            <RouterLink :to="{ name: 'recordPage' }" active-class="category-action" class="relative">
              <img :src="`/assets/images/rebate.png`" alt="" class="w-full">
              <h1 v-if="!isCollapsed" class="button-title-menu">{{ $t('Records') }}</h1>
            </RouterLink>
          </div>
          <div>
            <RouterLink :to="{ name: 'vipPage' }" active-class="category-action" class="relative">
              <img :src="`/assets/images/vip.png`" alt="" class="w-full">
              <h1 v-if="!isCollapsed" class="button-title-menu">{{ $t('Vip') }}</h1>
            </RouterLink>
          </div>
          <div>
            <RouterLink :to="{ name: 'bonusPage' }" active-class="category-action" class="relative">
              <img :src="`/assets/images/coletas.png`" alt="" class="w-full">
              <h1 v-if="!isCollapsed" class="button-title-menu">{{ $t('Bonus') }}</h1>
            </RouterLink>
          </div>
        </div>
        <ul class="font-medium mt-5 mb-[200px]">
          <li class="px-3">
            ...
            >
              ...
              <span v-if="!isCollapsed" class="ml-3">{{ $t('Support') }}</span>
            </a>
          </li>
          <li class="px-3">
            <a
            ...
            >
              ...
              <span v-if="!isCollapsed" class="ml-3">{{ $t('Promotions') }}</span>
            </a>
          </li>
          <li class="px-3">
            <a
            ...
            >
              ...
              <span v-if="!isCollapsed" class="ml-3">{{ $t('Refer a friend') }}</span>
            </a>
          </li>
          <li class="px-3">
            <a
              @click="$router.push('/support-center')"
              href="#"
            ...
            >
              ...
              <span v-if="!isCollapsed" class="flex-1 ml-3 whitespace-nowrap">{{ $t('Support Center') }}</span>
            </a>
          </li>
        </ul>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      getSetting() {
        const _this = this
        const settingStoreData = useSettingStore()
        const settingData = settingStoreData.setting
        if (settingData) {
          _this.setting = settingData
        }
      },
